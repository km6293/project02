...
<head>
    ...
    <link href="../food-shop/css/nouislider.min.css" rel="stylesheet">
    <link href="../food-shop/css/range-slider.css" rel="stylesheet">
...
제목 ${dto.title} <br/>
아이디 ${dto.id} <br/>
작성자 <a href="/azaz/sell/list2.do?id=${dto.id}"> ${dto.writer}</a>

<c:if test="${c.rating == 1}">
	<input type="button" class="btn web-btn rounded-pill" value="즐겨찾기" onclick="window.location='/azaz/mylist/listInsert.do?name=${dto.id}&my_id=${sessionScope.memId}&mnum=${dto.num}'">
</c:if>

<input type="button" class="btn web-btn rounded-pill" value="업체위치" onclick="window.location='/azaz/user/azazMap.do?id=${dto.id}'"><br/>
사진 <br/><img src="/azaz/img/${dto.img}"><br/>

내용 ${dto.content} <br/>
조회수 ${dto.readcount}<br/>


<input type="button" class="btn web-btn rounded-pill" value="뒤로가기" onclick="javascript:window.location='list.do'"><br/><br/>

<c:if test="${sessionScope.memId == dto.id}">
	<button class="btn web-btn rounded-pill" type="button" onclick="location.href='/azaz/sell/update.do?num=${dto.num}'">수정</button>
	<button class="btn web-btn rounded-pill" type="button" onclick="location.href='/azaz/sell/delete.do?num=${dto.num}'">삭제</button>
</c:if>
...
<c:forEach items="${reply}" var="reply">
		...
		<c:if test="${sessionScope.memId == reply.id}">
	       <input type="button" class="btn web-btn rounded-pill" value="수정" onclick="window.location='replyUpdate.do?re_no=${reply.re_no}&num=${dto.num}'"> 
	       <input type="button" class="btn web-btn rounded-pill" value="삭제" onclick="window.location='replydelete.do?re_no=${reply.re_no}&num=${dto.num}'">
		</c:if>   
    </c:forEach>
...
<form action="replyinsert.do" method="post">    
 	...
 	<input type="hidden" id="num" name="num" value="${dto.num}">
        ...
        <input type="submit" class="btn web-btn rounded-pill" value="댓글 달기">
</form>
